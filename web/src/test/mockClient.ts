...
const bootstrap: Bootstrap = { account:{displayName:"Test",initials:"TB"},workspaces:[{id:"ws",name:"Data engineering",archived:false},{id:"finance",name:"Финансы",archived:false}],activeWorkspaceId:"ws",subscriptions:[{id:"sub",name:"This Week in Rust",count:3,status:"active",lastUpdate:"now"}],articles,newArticleCount:0 };

export function mockClient(overrides: Record<string, unknown> = {}) {
  const transport: Transport = async <T>(path: string, init?: RequestInit) => {
    if (path === "/api/bootstrap") return bootstrap as T;
