...
function recordingClient(){
 ...
 const bootstrap:Bootstrap={account:{displayName:"Test",initials:"T"},workspaces:[{id:"ws",name:"Data engineering",archived:false},{id:"finance",name:"Финансы",archived:false}],activeWorkspaceId:"ws",subscriptions:[{id:"sub",name:"This Week in Rust",count:1,status:"active",lastUpdate:"now"}],articles,newArticleCount:0};
 const transport:Transport=async<T,>(path:string,init?:RequestInit)=>{
  calls.push({path,init}); if(path==="/api/bootstrap")return bootstrap as T;
  if(path.startsWith("/api/articles?"))return [{...articles[0],id:"finance",title:"Finance workspace article"}] as T;
...
describe("workspace and subscription isolation",()=>{
 it("targets pause at the selected subscription and sends the exact reason",async()=>{
  const calls:{path:string;init?:RequestInit}[]=[];
  const bootstrap:Bootstrap={account:{displayName:"Test",initials:"T"},workspaces:[{id:"ws",name:"Data",archived:false}],activeWorkspaceId:"ws",subscriptions:[{id:"first",name:"Same name",count:1,status:"active",lastUpdate:"now"},{id:"second",name:"Same name",count:1,status:"active",lastUpdate:"now"}],articles:[{...articles[0],subscriptionIds:["second"]}],newArticleCount:0};
  const transport:Transport=async<T,>(path:string,init?:RequestInit)=>{calls.push({path,init});if(path==="/api/bootstrap")return bootstrap as T;return undefined as T;};
 ...
 it("scopes mark-all to the selected subscription",async()=>{
  const calls:{path:string;init?:RequestInit}[]=[];
  const bootstrap:Bootstrap={account:{displayName:"Test",initials:"T"},workspaces:[{id:"ws",name:"Data",archived:false}],activeWorkspaceId:"ws",subscriptions:[{id:"chosen",name:"Chosen feed",count:1,status:"active",lastUpdate:"now"}],articles:[{...articles[0],id:"chosen-article",subscriptionIds:["chosen"]},{...articles[1],id:"other-article",subscriptionIds:["other"]}],newArticleCount:0};
  const transport:Transport=async<T,>(path:string,init?:RequestInit)=>{calls.push({path,init});if(path==="/api/bootstrap")return bootstrap as T;return undefined as T;};
...
describe("subscription lifecycle",()=>{
 it("only offers recipe editing for editable Web feeds",async()=>{
  const bootstrap:Bootstrap={account:{displayName:"Test",initials:"T"},workspaces:[{id:"ws",name:"Data",archived:false}],activeWorkspaceId:"ws",subscriptions:[{id:"rss",name:"RSS",count:0,status:"active",editableWebFeed:false},{id:"web",name:"Web",count:0,status:"active",editableWebFeed:true}],articles:[],newArticleCount:0};
  const transport:Transport=async<T,>(path:string)=>path==="/api/bootstrap"?bootstrap as T:undefined as T;
 ...
 it("renames and unsubscribes without removing archived articles, then exposes restore",async()=>{
  const calls:{path:string;init?:RequestInit}[]=[];
  const bootstrap:Bootstrap={account:{displayName:"Test",initials:"T"},workspaces:[{id:"ws",name:"Data",archived:false}],activeWorkspaceId:"ws",subscriptions:[{id:"sub",name:"Original",count:1,status:"active",lastUpdate:"now"}],articles:[{...articles[0],subscriptionIds:["sub"]}],newArticleCount:0};
  const transport:Transport=async<T,>(path:string,init?:RequestInit)=>{calls.push({path,init});if(path==="/api/bootstrap")return bootstrap as T;if(path==="/api/subscriptions/sub"&&init?.method==="PATCH")return {...bootstrap.subscriptions[0],name:"Renamed"} as T;if(path==="/api/subscriptions/sub"&&init?.method==="DELETE")return {...bootstrap.subscriptions[0],name:"Renamed",status:"archived"} as T;if(path==="/api/subscriptions/sub/restore")return {...bootstrap.subscriptions[0],name:"Renamed",status:"active"} as T;return undefined as T;};
